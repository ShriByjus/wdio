...
const draftOrderMockData = require('../../../../data/byjus-classes/single-order/cbse/4th-grade-Order/draftOrder.json');
const salesOrderMockData = require('../../../../data/byjus-classes/single-order/cbse/4th-grade-Order/salesOrder.json');
const salesSubOrderMockData = require('../../../../data/byjus-classes/single-order/cbse/4th-grade-Order/salesSubOrder.json');
...
describe('Validating Byjus classes single order for 4th standard', async () => {
    ...
    it('Verify the Addon Items Skus', async () => {
        allure.startStep("Wait for the orders tab to be clickable");
        await verifyOmsOrderDetails.ordersTab.waitForClickable({ timeout: 10000 });
        allure.startStep("Click on orders tab");
        await verifyOmsOrderDetails.ordersTab.click();
        allure.startStep("Click on sales Orders Tab tab");
        await verifyOmsOrderDetails.salesOrdersTab.click();
        allure.startStep("Enter the Sales order Id ins search barch");
        await verifyOmsOrderDetails.tfSearchBarSsoProducts.setValue(salesOrderId);
        allure.startStep("Click on sales Orders Id");
        await verifyOmsOrderDetails.btnOpenSalesOrderId(salesOrderId).click();
        allure.startStep("Copy Add On Items skus from Csv file");
        let addOnItemsSkusCsv = await verifyOmsOrderDetails.copyingAddOnItemsSkusFromCsv(1, process.env.ADD_ONN_ITEMS_SKUS_CSV);
        allure.startStep("Copy Add On Items sku names from UI");
        await verifyOmsOrderDetails.getAddOnItemSkuName(addOnItemsSkusCsv.skuIds[0]).waitForDisplayed({ timeout: 30000 });
        let addOnItemsSkus = await verifyOmsOrderDetails.getAddOnItemSkuName(addOnItemsSkusCsv.skuIds[0]).getText();
        allure.startStep("Validate the Add On Items sku names");
        expect(addOnItemsSkus).toEqual(addOnItemsSkusCsv.skuName[0]);
        allure.endStep();
    })
    ...
    it('Verify the Shipemnt skus', async () => {
        ...
        let shipmentSkusCsv = await verifyOmsOrderDetails.copyingShipmentSkusFromCsv(0, process.env.SHIPMENT_SKUS_BYJUS_CLASSES_CSV);
    ...
    it('Verify the sales skus', async () => {
        ...
        allure.startStep("Copy shipment skus from Csv file");
        let salesSkusCsv = await verifyOmsOrderDetails.copyingSalesSkusFromCsv(0, process.env.SALES_SKUS_BYJUS_CLASSES_CSV);
    ...
    it('Verify PID is generated', async () => {
        ...
        allure.startStep("Scroll to PID button");
        await verifyOmsOrderDetails.verifyPid.waitForClickable({ timeout: 30000 });
