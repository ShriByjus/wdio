...
const draftOrderMockData = require('../../../../../../data/btlpData/siblingOrders.js/twoSiblingOrder/ukgOrder/cbse/draftOrder.json');
const salesOrderMockData = require('../../../../../../data/btlpData/siblingOrders.js/twoSiblingOrder/ukgOrder/cbse/salesOrder.json');
const salesSubOrderMockData = require('../../../../../../data/btlpData/siblingOrders.js/twoSiblingOrder/ukgOrder/cbse/salesSubOrder.json');
...
describe('Validating UKG-LKG Two sibling order', async () => {
    ...
    it('Verify the Addon Items Skus', async () => {
        ...
        let addOnItemsSkusCsv = await verifyOmsOrderDetails.copyingAddOnItemsSkusFromCsv(2, process.env.ADD_ONN_ITEMS_SKUS_CSV);
        let newSkuIdArr = await duplicate(addOnItemsSkusCsv.skuIds, 2);
        let newSkuNameArr = await duplicate(addOnItemsSkusCsv.skuName, 2);
        allure.startStep("Find out the length of Add On Items sku array");
        let addOnItemsSkuCount = newSkuIdArr.length;
        allure.startStep("Copy Add On Items sku names from UI");
        let addOnItemsSkus = await verifyOmsOrderDetails.copyAddOnItemsSkuNames(salesOrderId, addOnItemsSkuCount, newSkuIdArr);
        allure.startStep("Validate the Add On Items sku names");
        expect(addOnItemsSkus).toEqual(newSkuNameArr);
        allure.endStep();
    })
    ...
    it('Verify the Shipemnt skus', async () => {
        ...
        let shipmentSkusCsv = await verifyOmsOrderDetails.copyingShipmentSkusFromCsv(3, process.env.SHIPMENT_SKUS_BTLP_CSV);
    ...
    it('Verify the sales skus', async () => {
        ...
        allure.startStep("Copy shipment skus from Csv file");
        let salesSkusCsv = await verifyOmsOrderDetails.copyingSalesSkusFromCsv(3, process.env.SALES_SKUS_BTLP_CSV);
