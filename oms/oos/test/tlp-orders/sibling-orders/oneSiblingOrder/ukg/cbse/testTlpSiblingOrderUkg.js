...
const draftOrderMockData = require('../../../../../../data/btlpData/siblingOrders.js/singleSiblingOrder/ukgOrder/cbse/draftOrder.json');
const salesOrderMockData = require('../../../../../../data/btlpData/siblingOrders.js/singleSiblingOrder/ukgOrder/cbse/salesOrder.json');
const salesSubOrderMockData = require('../../../../../../data/btlpData/siblingOrders.js/singleSiblingOrder/ukgOrder/cbse/salesSubOrder.json');
...
let count = 0;
describe('Validating UKG-LKG Single sibling order', async () => {
    ...
    it('Verify the Addon Items Skus', async () => {
        ...
        let addOnItemsSkuCount = addOnItemsSkusCsv.skuIds.length;
        allure.startStep("Copy Add On Items sku names from UI");
        let addOnItemsSkus = await verifyOmsOrderDetails.copyAddOnItemsSkuNames(salesOrderId, addOnItemsSkuCount, addOnItemsSkusCsv.skuIds);
        allure.startStep("Validate the Add On Items sku names");
        expect(addOnItemsSkus).toEqual(addOnItemsSkusCsv.skuName);
        allure.endStep();
    })
    ...
    it('Verify the Shipemnt skus', async () => {
        ...
        let shipmentSkusCsv = await verifyOmsOrderDetails.copyingShipmentSkusFromCsv(2, process.env.SHIPMENT_SKUS_BTLP_CSV);
    ...
    it('Verify the sales skus', async () => {
        ...
        allure.startStep("Copy shipment skus from Csv file");
        let salesSkusCsv = await verifyOmsOrderDetails.copyingSalesSkusFromCsv(2, process.env.SALES_SKUS_BTLP_CSV);
    ...
    it('Verify the sum of sku values should match the sub total amount', async () => {
        ...
        await verifyOmsOrderDetails.btnToggleToSalesSku.click();
        allure.startStep("Copy shipment skus from Csv file");
        let salesSkusCsv = await verifyOmsOrderDetails.copyingSalesSkusFromCsv(1, process.env.SALES_SKUS_BTLP_CSV);
        allure.startStep("Find out the length of sales sku array");
        let salesSkuCount = salesSkusCsv.skuIds.length;
